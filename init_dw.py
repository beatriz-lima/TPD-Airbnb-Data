...
def insert_data(df, table_name, conn, facts=['booking', 'listings']):

    df = pd.read_csv('processed_dt/{}.csv'.format(df))
...
-- Create DW
CREATE DATABASE {};
""".format(dbconnection.dbname, dbconnection.dbname)

conn = psycopg2.connect(host = dbconnection.server_host,database = 'postgres', user = dbconnection.dbusername,password = dbconnection.dbpassword,sslmode=dbconnection.sslmode)
conn.autocommit = True
success = run_sql_command(sql=create, conn=conn)
...
if success is True:
    ...
    success = run_sql_command(sql=schema, conn=conn)
    ...
    if success is True:
        ...
        inputs = [('location', 'location'), ('property', 'property'), ('host', 'df_host_processed'), ('date', 'dimension_data'), ('review', 'review'), ('booking', 'bookings_fact'), ('listings', 'listings_fact')]
        
        for i in inputs:
        
            conn = psycopg2.connect(host = dbconnection.server_host,database = dbconnection.dbname, user = dbconnection.dbusername,password = 
                                    dbconnection.dbpassword,sslmode=dbconnection.sslmode)
            success = insert_data(i[1],i[0], conn)
            conn.close()
            if success is False:
                print('Data Not Inserted')
                break
            else:
                print('Data insert on table {}'.format(i[0]))
        
        if success is True:
            print('Data Inserted')
            print('Database Iniciated Successively')
